...
ArrayList* createList(int initialCapacity)
{
    ...
    list->array = (int*)malloc(initialCapacity * sizeof(int));	//리스트 배열을 크기만큼 동적할당 
    list->size = 0;
...
void resizeList(ArrayList* list)
{
    ...
    list->array = (int*)realloc(list->array, list->capacity * sizeof(int)); //리스트 배열을 대상으로 새로운 동적할당(새로운 크기)
}
...
void addElement(ArrayList* list, int element)
{
    ...
    list->array[list->size++] = element; //리스트 배열의 새로운 위치에 데이터 저장
}
...
void removeElement(ArrayList* list, int index)
{
    if (index < 0 || index >= list->size) {
        ...
        return;
    } //탐색대상이 0보다 작거나 또는 참조 위치보다 크거나 같으면 삭제 실패
    for (int i = index; i < list->size - 1; i++) {
        list->array[i] = list->array[i + 1];
    } //삭제할 리스트 배열의 위치(i)를 다음 배열의 위치(i+1)로 변경
    list->size--; //리스트 배열의 저장된 데이터 수 감소
}
