...
* 단점
  * 인덱스를 통한 접근이 느리다. (O(n))
  * 노드가 한 방향으로만 연결되어 있어, 노드를 삭제하거나 삽입할 때 이전 노드에 대한 정보를 미리 알고 있어야 함.
...
#### 2-3. 다중 연결리스트 (Multilevel Linked List)
* 하나의 연결리스트에 여러 레벨로 다른 연결리스트가 연결된 구조로서, 한 노드가 또 다른 연결리스트의 시작을 가리킬 수 있고 이를 통해 다차원 데이터를 표현할 수 있다.

* 특징: 각 노드가 하위 리스트를 가리킬 수 있음.
* 사용 사례: 2차원 배열, 트리 구조 등의 데이터를 표현할 때 사용 가능.

  
#### 2-4. 원형 연결 리스트(Circular Linked List)
...
* 특징
  * 각 노드는 **prev**와 next 포인터를 가짐.
  * 리스트를 양방향으로 탐색 가능.
  * 삽입 및 삭제 연산이 단방향 연결리스트보다 더 유연함.
* 장점
  * 양방향으로 탐색이 가능해 데이터를 처리할 때 효율적임.
  * 삽입과 삭제 연산 시, 단방향 연결리스트보다 더 간단하게 수행 가능.
* 단점
  * 각 노드가 두 개의 포인터(prev와 next)를 가지므로, 메모리 사용량이 더 많음.
  * 구현이 단방향 연결리스트보다 복잡함.
